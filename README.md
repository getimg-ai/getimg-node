...
await client.images.generate({ model: 'seedream-5-lite', prompt: 'A cinematic portrait of a cat astronaut' }, {
  maxRetries: 5,
});
```
...
const { data: response, response: raw } = await client.images
  .generate({ model: 'seedream-5-lite', prompt: 'A cinematic portrait of a cat astronaut' })
  .withResponse();
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/getimg-typescript/issues) with questions, bugs, or suggestions.
